...
class Ship:
    ...
    def move(self):
        if pyxel.btn(pyxel.KEY_LEFT) and self.x > MIN_X:
            self.x -= 1
        if pyxel.btn(pyxel.KEY_RIGHT) and self.x < MAX_X - 4:
            self.x += 1
...
class Alian:
    ...
    def formation(self):
        ...
        if random.randint(1, 2) == 1:
            ...
            if self.alians[35]["alive"]:
                ...
                togather += 1
            if togather < 2 and self.alians[36]["alive"]:
    ...
    def hitByBeam(self, pos):
        ...
        for i in range(len(self.attacks)-1, -1, -1):
            if (self.attacks[i]["x"] <= beam_x <= self.alians[i]["x"]+4) and\
...
class Point:
    ...
    def draw(self):
        for point in self.points:
            pyxel.text(
                point[2]*2+BEGIN_X+5, point[3]*5+5,
                str(point[0]), 15
            )
...
class Sound:
    def __init__(self):
        #宇宙船の爆発音
        pyxel.sound(1).set("f3f1", "n", "7", "s", 15)
        #エイリアンの爆発音
        pyxel.sound(2).set("c2a4c0", "n", "4", "s", 6)
        #フォーメーション攻撃中の音
        st = ""
        for i in "432":
            for s in "bagfedc":
                st += s+i
        pyxel.sound(3).set(st, "sp", "2", "s", 16)
...
class Galaxian:
    def __init__(self):
        #pyxelの初期化
        pyxel.init(*OUTER_SIZE, caption="Galaxian")

        #各キャラクタの準備(オブジェクトを”作る”)
